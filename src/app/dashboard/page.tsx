...
export default function DashboardPage() {
  ...
  async function updateSubject(subjectId: string) {
    const response = await fetch("/api/subjects", { method: "PATCH", headers: await authHeaders(true), body: JSON.stringify({ subjectId, name: editSubjectName }) });
    const result = await readApiResponse(response);
    if (!response.ok) { const error = String(result.error ?? "Unable to update subject."); setMessage(error); toast(error, "error"); return; }
    setSubjects((current) => current.map((subject) => subject.id === subjectId ? { ...subject, name: editSubjectName.trim() } : subject));
    setEditingSubject(null); toast("Subject updated and sheet tab renamed.", "success");
  }
  async function deleteSubject(subjectId: string) {
    if (!window.confirm("Delete this subject and its attendance sheet tab?")) return;
