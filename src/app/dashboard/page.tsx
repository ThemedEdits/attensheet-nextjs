...
export default function DashboardPage() {
  ...
  useEffect(() => onAuthStateChanged(firebaseAuth, async (user) => {
    ...
    const classes = nextProfile.role === "cr"
      ? await getDocs(query(collection(firestore, "classes"), where("crUid", "==", user.uid)))
      : null;
  ...
  async function createSubject() {
    ...
    if (!user || !classRecord || !subjectName.trim()) return;
    setMessage("");
